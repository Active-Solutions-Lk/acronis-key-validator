...
export async function ExtSpreadsheet(file, headers) {
    ...
    if (file.name.toLowerCase().endsWith('.csv')) {
      Papa.parse(file, {
        complete: (results) => {
          ...
          const validation = validateDataStructure(fileHeaders, jsonData);
          resolve({
      ...
      reader.onload = (e) => {
        try {
          ...
          const validation = validateDataStructure(fileHeaders, data);
          resolve({
            valid: validation.isValid,
            headers: fileHeaders,
            data,
